...
Migrated so far: `helping_handed_this_turn` → `VolatileKind::HelpingHand` (PR-169); `flinched_this_turn` → `VolatileKind::Flinch` (PR-170); `is_protected_this_turn` → `VolatileKind::Protect` (PR-171); `damaged_this_turn` → `VolatileKind::DamagedThisTurn` (PR-172); `switched_in_this_turn` → `VolatileKind::JustSwitchedIn` (PR-173); `pending_self_switch` → `VolatileKind::PendingSelfSwitch` (PR-174); `redirecting_this_turn` + `redirecting_is_powder` → `VolatileKind::Redirect` (PR-178, payload bit 0 = is_powder); `toxic_counter` → `VolatileKind::ToxicCounter` (PR-179, payload = counter); `substitute_hp` → `VolatileKind::Substitute` (PR-180, payload = hp); `sleep_turns` → `VolatileKind::Sleep` (PR-181, payload = turns); `encore_turns` + `encored_move_slot` → `VolatileKind::Encore` (PR-182, payload = turns<<8 | slot); `stall_counter` + `used_stall_this_turn` → `VolatileKind::Stall` (PR-183, payload = used<<8 | counter). All expose method-based accessors (`mon.is_protected_this_turn()` / `set_protected(bool)`), fields are gone from `Pokemon`, and the 4 struct-literal init sites are collapsed accordingly.

Continuation note (next session): next up `damaged_this_turn`, then `switched_in_this_turn`, then redirection, encore, sleep, toxic, substitute, stall. Each is its own atomic PR. Each migration must pass `cargo test --workspace --exclude vgc-engine-py` + corpus harness — net-neutral expected.
