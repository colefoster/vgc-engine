...
**Status**: shipped — PR-155 (`VolatileKind` enum with 30+ PS-named kinds, `Volatile { kind, turns_remaining: u8, payload: u32 }`, `VolatileSet` fixed-cap 8-slot registry with `add` / `remove` / `has` / `get` / `position` / `tick` / `clear`); migrations: `helping_handed_this_turn` → `HelpingHand` (PR-169); `flinched_this_turn` → `Flinch` (PR-170); `is_protected_this_turn` → `Protect` (PR-171); `damaged_this_turn` → `DamagedThisTurn` (PR-172); `switched_in_this_turn` → `JustSwitchedIn` (PR-173); `pending_self_switch` → `PendingSelfSwitch` (PR-174); `redirecting_this_turn` + `redirecting_is_powder` → `Redirect` (PR-178, payload bit 0 = is_powder); `toxic_counter` → `ToxicCounter` (PR-179, payload = counter); `substitute_hp` → `Substitute` (PR-180, payload = hp); `sleep_turns` → `Sleep` (PR-181, payload = turns); `encore_turns` + `encored_move_slot` → `Encore` (PR-182, payload = turns<<8 | slot); `stall_counter` + `used_stall_this_turn` → `Stall` (PR-183, payload = used<<8 | counter); `locked_move_slot` → `Locked` (PR-184, payload = slot). All accessors are method-based on `Pokemon`. Remaining ad-hoc fields (`crit_stage_volatile`, `semi_invuln`, `charging_turns`, `must_recharge`, `lockin_turns` + `lockin_move_slot`, `boosted_stat`, `booster_locked`, `turns_active`) are kept as direct fields — they're tightly coupled to a single mechanic each and don't benefit from the registry indirection.
...
**Status**: shipped — PR-164 (back-solve primitive), PR-166 (`DamageHint` event variant + `damage_roll_hint` method), PR-167 (wired into the battle.rs damage call site), PR-175 (`build_damage_oracle_for_turn` / `build_damage_oracle_for_replay` in `vgc-engine-replay::oracle`: walks `|-damage|` events, tracks per-slot running HP fraction from each Damage / Heal / Switch / Drag, and emits `RngEvent::DamageHint(observed_raw_hp)` for direct damaging hits — caller passes a `max_hp_for_slot` closure to convert the protocol's percent fraction into the engine's raw-HP units; indirect-damage entries update the running fraction but do not emit a hint; returns 0-HP lookups silently skip the hint so the queue stays coherent), PR-187 (`scorer::score_replay_oracle` switched to `build_oracle_for_replay` — crit + accuracy), PR-188 (`scorer::score_replay_full_oracle` concatenates crit + accuracy + damage events; CLI `score-corpus --full-oracle` flag; `max_hp_for_slot` walks `|switch|` / `|drag|` alongside the damage walker and resolves max HP from the reconstructed team).
...
**Status**: shipped — recon hooks shipped (PR-96/97/98); PR-165 added the `spread_recon` module with `narrow_by_damage(candidates, evidence, role)`; PR-176 added `recon_smogon::SpreadEvidenceObserver` — initialized from `SmogonStats::top_n` per species, consumes `|-damage|` observations via `observe_damage(...)` and shrinks each species' candidate set monotonically using `narrow_by_damage` under the hood. `surviving_top(species)` exposes the highest-weighted surviving spread (or `None` if narrowing emptied the set, signaling fall-back to the raw Smogon prior); PR-189 (`recon_smogon::EvidenceTunedRecon` + `build_evidence_recon(replay, base)` wraps a `SmogonStatsRecon`, pre-walks each replay's `|move|` / `|-damage|` stream to construct a `SpreadEvidenceObserver`, then overrides each species' (nature, EVs) with `surviving_top` at `reconstruct` time; `score-corpus` always wraps the Smogon recon with this — it can only narrow priors).
