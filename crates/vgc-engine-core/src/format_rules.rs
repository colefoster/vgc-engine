...
//!
//! Move legality is **skipped**: the engine does not surface learnset data
//! (`learnsets.json` is not ingested by `build.rs`). See
//! [`FormatRules::check_move_legality`].
...
pub struct FormatRules {
    ...
    /// When `false`, move legality is not checked (no learnset data surfaced).
    /// Move *count*, *duplicates*, and *unknown moves* are still validated.
...
pub const REG_M_B: FormatRules = FormatRules {
    ...
    // No learnset data surfaced (build.rs does not ingest learnsets.json).
...
pub fn verify_team(team: &[TeamMember], rules: &FormatRules) -> Result<(), Vec<Violation>> {
    ...
    for (i, m) in team.iter().enumerate() {
        ...
        let species = data::species_by_slug(&slugify(&m.species));
        ...
        // Move legality — skipped unless the format opts in (no learnset data).
        if rules.check_move_legality {
            // TODO: validate each move against the species learnset once
            // build.rs ingests learnsets.json.
        }
    }
