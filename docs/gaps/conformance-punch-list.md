...
- ✅ **Beat Up** — DONE (PR #24, 2026-06-24). Was fully broken (base_power 0,
  ...
  Atk-stat override): one hit per eligible party member (active user always
  counts, even if statused; others iff `!fainted && no status`), each
  Dark/physical hit using the ACTIVE user's Atk/level/STAB/ability/item with
  per-hit **BP = 5 + floor(member.species_base_atk/10)**. `calculate_damage` is
  now a wrapper over `calculate_damage_with_bp(.., Option<u32>)` (None
  byte-identical); the post-formula attacker-item multipliers were extracted
  into one shared closure (single source of truth — normal path stays exact,
