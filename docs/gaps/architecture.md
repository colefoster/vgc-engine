...
Migrated so far: `helping_handed_this_turn` → `VolatileKind::HelpingHand` (PR-169); `flinched_this_turn` → `VolatileKind::Flinch` (PR-170); `is_protected_this_turn` → `VolatileKind::Protect` (PR-171). All expose method-based accessors (`mon.is_protected_this_turn()` / `set_protected(bool)`), fields are gone from `Pokemon`, and the 4 struct-literal init sites are collapsed accordingly.
