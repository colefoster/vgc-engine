...
pub fn calculate_damage(
    ...
    let screen_applies = (ctx.defender_has_reflect && physical)
...
pub fn damage_range(attacker: &Pokemon, defender: &Pokemon, move_id: u16) -> (u16, u16) {
    let min = calculate_damage(
        ...
        defender,
        move_id,
        DamageContext { crit: false, roll: DamageContext::MIN_ROLL, is_spread: false, weather: crate::weather::Weather::None, defender_has_reflect: false, defender_has_light_screen: false, is_doubles: false },
    );
    let max = calculate_damage(
        attacker,
        defender,
        move_id,
        DamageContext { crit: false, roll: DamageContext::MAX_ROLL, is_spread: false, weather: crate::weather::Weather::None, defender_has_reflect: false, defender_has_light_screen: false, is_doubles: false },
    );
    (min, max)
}
...
mod tests {
    ...
    fn crit_increases_damage() {
        ...
        let crit = calculate_damage(&attacker, &defender, move_id("earthquake"),
            DamageContext { crit: true, roll: 15, is_spread: false, weather: crate::weather::Weather::None, defender_has_reflect: false, defender_has_light_screen: false, is_doubles: false });
        assert!(crit > no_crit);
